...
void LedgerImpl::PanelLoadPublisherList(
    uint64_t window_id,
    const std::string& publisher_key) {
  DownloadPublisherList(
      std::bind(&LedgerImpl::OnPanelPublisherListDownloaded,
                this,
                _1,
                _2,
                _3,
...
void LedgerImpl::OnPanelPublisherListDownloaded(
    int response_status_code,
    const std::string& response,
    const std::map<std::string, std::string>& headers,
    uint64_t window_id,
  ...
  bat_publishers_->RefreshPublisherVerifiedStatus(
      publisher_key,
      std::bind(&LedgerImpl::OnPanelPublisherInfo,
                this,
                _1,
                _2,
                window_id));
